...
$halaman_awal = ($halaman>1) ? ($halaman * $batas) - $batas : 0;	
  ...
  $next = $halaman + 1;
	...
	$data=mysqli_query($conn,"select m.nama_menu, p.total_pembayaran, p.tgl_pembayaran from pembayaran as p join menu as m join pesanan as ps WHERE ps.id_menu = m.id_menu and ps.id_pesanan = p.id_pesanan and p.id_pelanggan = '$dpp'");
$jumlah_data = mysqli_num_rows($data);
$total_halaman = ceil($jumlah_data / $batas);
$halaman_2akhir = $total_halaman - 1;
$adjacents = "2";

$data_produk = mysqli_query($conn,"select p.id_pembayaran, m.nama_menu, p.total_pembayaran, p.tgl_pembayaran, p.status from pembayaran as p join menu as m join pesanan as ps WHERE ps.id_menu = m.id_menu and ps.id_pesanan = p.id_pesanan and p.id_pelanggan = '$dpp' ORDER BY p.id_pembayaran DESC limit $halaman_awal, $batas");
				$nomor = $halaman_awal+1;
       ?> 
            <div class="row">
                <table class="table table-dark text-white text-center table-striped table-responsive-sm">
                <tr>
                    <th>No</th><th>ID Pembayaran</th><th>Nama Menu</th><th>Total Bayar</th><th>Aksi</th>
                </tr>
                <?php
                $no = 1;
