...
$halaman_awal = ($halaman>1) ? ($halaman * $batas) - $batas : 0;	
  ...
  $next = $halaman + 1;
	...
	$data=mysqli_query($conn,"select p.id_pembayaran, pl.nama_pelanggan, m.nama_menu, p.total_pembayaran, p.tgl_pembayaran, p.bukti, p.status, ps.jumlah_pesanan from pembayaran as p join menu as m join pesanan as ps join pelanggan as pl WHERE p.id_pelanggan = pl.id_pelanggan and ps.id_menu = m.id_menu and ps.id_pesanan = p.id_pesanan and p.status != 2");
$jumlah_data = mysqli_num_rows($data);
$total_halaman = ceil($jumlah_data / $batas);
$halaman_2akhir = $total_halaman - 1;
$adjacents = "2";

$data_produk = mysqli_query($conn,"select p.id_pembayaran, pl.nama_pelanggan, m.nama_menu, p.total_pembayaran, p.tgl_pembayaran, p.bukti, p.status, ps.jumlah_pesanan from pembayaran as p join menu as m join pesanan as ps join pelanggan as pl WHERE p.id_pelanggan = pl.id_pelanggan and ps.id_menu = m.id_menu and ps.id_pesanan = p.id_pesanan and p.status != 2 limit $halaman_awal, $batas");
				$nomor = $halaman_awal+1;
       ?> 
            <div class="row">
                <table class="table table-dark text-white text-center table-striped table-responsive-sm">
                <tr>
                    <th>No</th><th>ID Pembayaran</th><th>Nama Pelanggan</th><th>Nama</th><th>Jumlah</th><th>Total Bayar</th><th>Bukti Pembayaran</th><th>Aksi</th>
                </tr>
                <?php
                $no = 1;
