...
function dbConnect(){
	$db=new mysqli("localhost","root","","dbresto");// Sesuaikan dengan konfigurasi server anda.
	return $db;
}
...
function getStruk($id){
	$db=dbConnect();
	if($db->connect_errno==0){
		$res=$db->query("select p.id_pembayaran, pl.nama_pelanggan, m.nama_menu, p.total_pembayaran, p.tgl_pembayaran, ps.jumlah_pesanan, pl.alamat_pelanggan from pembayaran as p join menu as m join pesanan as ps join pelanggan as pl WHERE ps.id_menu = m.id_menu and ps.id_pesanan = p.id_pesanan and id_pembayaran = '$id'");
		if($res){
			if($res->num_rows>0){
				$data=$res->fetch_assoc();
				$res->free();
